...
// In: LPCSTR = color to parse (eg. "black" or "white")
...
COLORREF BBColor::parseLiteralColor(const char *color) {
	...
	if (l > 2 && l < 32)
	{
		...
		if ((p = strstr(buf, "grey")) != NULL) {
			p[2]='a';
		}

		char c;
		int i;
		if (memcmp(buf, "gray", 4) == 0 && (c = buf[4]) >= '0' && c <= '9') {
			i = atoi(buf+4);
			if (i >= 0 && i <= 100) {
				i = (i * 255 + 50) / 100;
				return RGB(i, i, i);
		...
		int n;
		p = &buf[l - 2]; //p is the last "real" character in color
		i = *p - '1';
		if (i >= 0 && i < 4) { //is p a number between 0 and 3 inclusive?
			*p = 0;
			--l;
			struct litcolor4 *cp4 = litcolor4_ary;
			n = sizeof(litcolor4_ary) / sizeof(*cp4);
			do {
				if (memcmp(buf, cp4->cname, l) == 0) {
					return cp4->cref[i];
				}
				cp4++;
			} while (--n);
	...
	return (COLORREF)-1; //Didn't find a match, return -1
}
...
COLORREF BBColor::readColorFromString(const char *colorstring) {
	...
	if (!invalid) {
		...
		// Windows uses bgr instead of rgb
		return switchRgb(cr);
	} else {
...
COLORREF BBColor::switchRgb(COLORREF c) {
...
std::string BBColor::colorRefToString(COLORREF c) {
	...
	oss << (r < 15 ? "0" : "") << std::hex << r << (g < 15 ? "0" : "") << g << (b < 15 ? "0" : "") << b;
